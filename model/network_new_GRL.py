...
class GFNet(nn.Module):
    def __init__(self,
        ...
        self.domain_classifier = DomainDiscriminator(in_channels=64)
    ...
    def forward(self, batch, global_step=0, symmetric=False, upsample=False, scale_factor=1, pre_corresps=None, visualization=False):
        ...
        for idx, scale in enumerate(features0.keys()):
            ...
            if scale == '8':
                ...
                # 判别器预测
                domain_pred_f0 = self.domain_classifier(f0_rev)  # 应该预测 0
                domain_pred_f1 = self.domain_classifier(f1_rev)  # 应该预测 1

...
from model.CANN_GRL import DomainDiscriminator, grad_reverse
...
class GFNet(nn.Module):
    def __init__(self,
        ...
        # 域分类器
        # 假设 feature_extractor 输出的特征通道数是 128
        self.domain_classifier_8 = DomainDiscriminator(in_channels=64)
        self.domain_classifier_4 = DomainDiscriminator(in_channels=32)
        self.domain_classifier_2 = DomainDiscriminator(in_channels=16)
        self.domain_classifier_1 = DomainDiscriminator(in_channels=8)
        self.domain_classifier = [self.domain_classifier_8, self.domain_classifier_4, self.domain_classifier_2, self.domain_classifier_1]
    ...
    def forward(self, batch, global_step=0, symmetric=False, upsample=False, scale_factor=1, pre_corresps=None, visualization=False):
        im0 = batch["im_A"]
        im1 = batch["im_B"]
        corresps = {}
        B, C, H0, W0 = im0.shape
        ...
        for idx, scale in enumerate(features0.keys()):
            ...
            domain_pred_f1 = []
            if scale != '16':
                # 在 trainer 中计算当前的 alpha
                # total_steps = 2_000_000  # 2M pairs
                # warmup_steps = total_steps * 0.1
                # if global_step <= warmup_steps:
                #     alpha = 0.0
                # else:
                #     p = float(global_step - warmup_steps) / (total_steps - warmup_steps)
                #     alpha = 2. / (1. + np.exp(-10 * p)) - 1
                total_steps = 2_000_000  # 2M pairs
                p = float(global_step) / total_steps
                alpha = 2. / (1. + np.exp(-10 * p)) - 1
                f0_rev = grad_reverse(f0, alpha)
                f1_rev = grad_reverse(f1, alpha)
                # 判别器预测
                domain_pred_f0.append(self.domain_classifier[classifier_idx](f0_rev))  # 应该预测 0
                domain_pred_f1.append(self.domain_classifier[classifier_idx](f1_rev))  # 应该预测 1
                classifier_idx += 1
        ...
        return [corresps, domain_pred_f0, domain_pred_f1]
    ...
    def match(self, im0, im1, *args, batched = True):
        ...
        B,C,H1,W1 = im1.shape
        self.train(False)
        match_out = self.forward({"im_A":im0, "im_B":im1}, symmetric=self.symmetric)
        corresps = match_out[0]
        ...
        if self.upsample_preds:
            ...
            match_out = self.forward(batch, pre_corresps=finest_corresps, scale_factor=scale_factor, upsample=True, symmetric=self.symmetric)
            corresps = match_out[0]

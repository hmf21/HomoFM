...
class RobustLosses(nn.Module):
    ...
    def regression_loss(self, x2, prob, flow, certainty, scale, eps=1e-8, mode="delta"):
        ...
        for num_itr in flow.keys():
            ...
            gt_cert = prob
            ce_loss = ce_loss + self.iteration_base ** (len(flow) - num_itr) * F.binary_cross_entropy_with_logits(
                certainty[num_itr][:, 0], gt_cert)

            x = epe[prob > 0.99]
            reg_loss = reg_loss + self.iteration_base ** (len(flow) - num_itr) * cs ** a * (
                        (x / (cs)) ** 2 + 1 ** 2) ** (a / 2)
    ...
    def forward(self, match_out, batch, global_step):
        corresps = match_out[0]
        scales = list(corresps.keys())
        domain_pred_f0 = match_out[1]
        domain_pred_f1 = match_out[2]
        ...
        if global_step > warmup_steps:
            for domain_pred_f0_, domain_pred_f1_ in zip(domain_pred_f0, domain_pred_f1):
                loss_domain_0 = F.binary_cross_entropy_with_logits(
                    domain_pred_f0_, torch.zeros_like(domain_pred_f0_)
                )
                # d_pred1 的标签全是 1
                loss_domain_1 = F.binary_cross_entropy_with_logits(
                    domain_pred_f1_, torch.ones_like(domain_pred_f1_)
                )
                domain_loss = (loss_domain_0 + loss_domain_1) * 0.5
                tot_loss += domain_loss * 0.01
        ...
        for scale in scales:
            ...
            if scale == 'gm':
                ...
                reg_loss = self.ce_weight * loss[f"delta_certainty_loss_{scale}"] + loss[
                    f"delta_regression_loss_{scale}"]

    ...
    def __init__(self,
        ...
        if mode == 'train':
            if dataset == 'vis_ir_drone':
                ...
                train_list = open(f'{path}/train_list.txt').read().split('\n')
                ...
                path = f'{configs.cfg.DATA_PATH}/s3ma'
                # test_list = open(f'{path}/test_list_original.txt').read().split('\n')
                # all_list = os.listdir(f'{path}/train/trainimg/')
                # train_list = [x for x in all_list if x not in test_list][:5000]
                train_list = glob.glob(os.path.join(f'{path}/train/database_query_sat', '*'))
                for image_name_ in train_list:
                    image_name = os.path.basename(image_name_)
                    if torch.rand(1)>0.5:
                        imgs0.append(f'{path}/train/database_query_sat/' + image_name)
                        imgs1.append(f'{path}/train/queries_ir/' + image_name) ## r
                    else:
                        imgs0.append(f'{path}/train/queries_ir/' + image_name) ## r
                        imgs1.append(f'{path}/train/database_query_sat/' + image_name)
        elif mode == 'val':
